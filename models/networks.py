# This code is heavily borrowed and modified from the official BicycleGAN implementation
...
def get_scheduler(optimizer, opt, cur_ep):
    ...
    if opt.lr_policy == 'none':
        ...
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch - opt.n_ep_decay) / float(opt.n_ep1 + opt.n_ep2 - opt.n_ep_decay + 1)
            return lr_l
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule, last_epoch=cur_ep)
    elif opt.lr_policy == 'step':
        scheduler = lr_scheduler.StepLR(optimizer, step_size=opt.n_ep_decay, gamma=0.1, last_epoch=cur_ep)
    elif opt.lr_policy == 'plateau':
        scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.2, threshold=0.01, patience=5, last_epoch=cur_ep)
    elif opt.lr_policy == 'cosine':
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.n_ep1 + opt.n_ep2, eta_min=0, last_epoch=cur_ep)
    else:
...
def define_G(input_nc, output_nc, nz, ngf, netG='unet_128', norm='batch', nl='relu',
             use_dropout=False, init_type='xavier', init_gain=0.02, gpu_ids=[], where_add='input', upsample='bilinear', padding_type='zero'):
    net = None
    ...
    if netG == 'unet_128' and where_add == 'input':
        net = G_Unet_add_input(input_nc, output_nc, nz, 7, ngf, norm_layer=norm_layer, nl_layer=nl_layer,
                               use_dropout=use_dropout, upsample=upsample, padding_type=padding_type)
...
def define_E(input_nc, output_nc, ndf, netE,
    ...
    if netE == 'resnet_128':
        ...
        net = E_NLayers(input_nc, output_nc, ndf, n_layers=5, norm_layer=norm_layer,
                        nl_layer=nl_layer, vaeLike=vaeLike)
    elif netE == 'vgg_relu41':
        from models.networksadain import VggEncoder
        net = VggEncoder(output_nc, vaeLike)
        if len(gpu_ids) > 0:
          assert(torch.cuda.is_available())
          net.to(gpu_ids[0])
          net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs
...
class D_NLayersMulti(nn.Module):
    ...
    def get_layers(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        sequence = [nn.Conv2d(input_nc, ndf, kernel_size=kw,
                              stride=2, padding=padw), nn.LeakyReLU(0.2, False)]
...
class G_Unet_add_input(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
        ...
        unet_block = UnetBlock(ngf * max_nchn, ngf * max_nchn, ngf * max_nchn,
                               innermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, padding_type=padding_type)
        for i in range(num_downs - 5):
            unet_block = UnetBlock(ngf * max_nchn, ngf * max_nchn, ngf * max_nchn, unet_block,
                                   norm_layer=norm_layer, nl_layer=nl_layer, use_dropout=use_dropout, upsample=upsample, padding_type=padding_type)
        unet_block = UnetBlock(ngf * 4, ngf * 4, ngf * max_nchn, unet_block,
                               norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, padding_type=padding_type)
        ...
        unet_block = UnetBlock(input_nc + nz, output_nc, ngf, unet_block,
                               outermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, padding_type=padding_type)
...
class G_Unet_add_all(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
        ...
        unet_block = UnetBlock_with_z(
            ngf, ngf, ngf * 2, nz, unet_block, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample, padding_type=padding_type)
