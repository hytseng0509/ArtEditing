...
def main():
  ...
  if opts.load != '':
    ...
    opts.load_inference = join(opts.output_dir, 'model', opts.load_inference)
    print('  load workflow inference model file from {}'.format(opts.load_inference))
    model.load_inference(opts.load_inference)
    ep0 = -1
    total_it = 0
  model.set_train_mode('forward')
  model.set_scheduler(opts, ep0)
  ep0 += 1

  # train
  print('\n--- train artwork generation model ---')
